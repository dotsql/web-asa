...
class Martikel extends Model
{
    ...
    public function datatampilsearch($key){
        return $this->t_artikel->join('kategori','kategori.id_kategori=artikel.id_kategori_artikel','left')->where('spam_artikel',0)->like('judul_artikel',$key)->orderby('id_artikel','desc')->get();
    }
    ...
    public function datafiltersearch($key,$show,$page){
        $newpage= $page-1;
        $mulai= $newpage*$show;
        $batas= $show;
        return $this->t_artikel->join('kategori','kategori.id_kategori=artikel.id_kategori_artikel','left')->like('judul_artikel',$key)->where('spam_artikel',0)->orderby('id_artikel','desc')->limit($batas,$mulai)->get();
    }
